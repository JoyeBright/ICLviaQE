...
# Carbon tracker
tracker = CarbonTracker(epochs=2000, monitor_epochs=-1, epochs_before_pred = -1, verbose=2, components="gpu", log_dir=f'decoding/{domain},')
...
for it in range(num_itrs):
    ...
    for i, sample in tqdm(enumerate(eval_src)):
        ...
        with open(f"{domain}{'/task-level-XGLM-7.5b-'}{k}{'+0'}.{it}.new", "a") as f:
    ...
    with open(f"{domain}{'/task-level-XGLM-7.5b-'}{k}{'+0'}.{it}.new.{'bleu'}", "w") as f:
            f.write(str(score(outputs, eval_tgt)))
    scores["id"].append(it)
    scores["score"].append(score(outputs, eval_tgt)['score'])
    print(score(outputs, eval_tgt))
    predictions = []
    print("the id for the best-performing sample so far is:", scores["id"][np.argmax(scores["score"])])

print("the id for the best-performing sample is:", scores["id"][np.argmax(scores["score"])])
...
output_file_path = f"{domain}{'/task-level-XGLM-7.5b-'}{k}{'+0'}.{it}.new.TT"
